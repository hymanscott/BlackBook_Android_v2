...
public class DefaultAppLock extends AbstractAppLock {
    ...
    DatabaseHelper db;
    private Activity currentApp; //Keep a reference to the app that invoked the locker
    private SharedPreferences settings;
    private Date lostFocusDate;

    public DefaultAppLock(RegLogin currentApp) {
        super();
...
/*
    	...
    	} else if (settings.contains(String.valueOf(R.string.password_preference_key))) {
    		//read the password from the new key
    		storedPassword = settings.getString(String.valueOf(R.string.password_preference_key), "");
    		storedPassword = decryptPassword(storedPassword);
    		password = R.string.password_salt + password +  R.string.password_salt;
    	}
*/
//        setPassword("1234");
        ...
        //storedPassword =settings.getString(String.valueOf(R.string.password_preference_key), "");
        //storedPassword = decryptPassword(storedPassword);
        storedPassword = (String.valueOf(LynxManager.decryptString(LynxManager.getActiveUser().getPasscode())));
        storedPassword = R.string.password_salt + storedPassword + R.string.password_salt;
        password = R.string.password_salt + password + R.string.password_salt;

        Log.v("Passcode Test", password + " " + storedPassword);
    ...
    public boolean setPassword(String password) {
        ...
        if (password == null) {
            ...
            editor.remove(String.valueOf(R.string.password_preference_key));
            editor.commit();
            this.disable();
        } else {
            password = R.string.password_salt + password + R.string.password_salt;
            password = encryptPassword(password);
            editor.putString(String.valueOf(R.string.password_preference_key), password);
            editor.remove(OLD_APP_LOCK_PASSWORD_PREF_KEY);
            editor.commit();
            this.enable();
        }
    ...
    public boolean isPasswordLocked() {
        ...
        if (settings.contains(String.valueOf(R.string.password_preference_key)))
            return true;
